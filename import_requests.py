...
# INTERFAZ STREAMLIT
# ========================
st.set_page_config(page_title="📺 Programación Partidos y Canales en Vivo", layout="wide")
...
.canal-btn { width: 60%; text-align: left; font-size: 16px; background-color: #87CEFA; color: #000; border: none; border-radius: 8px; padding: 10px; margin: 5px auto; display: block; cursor: pointer; box-shadow: 0px 2px 4px rgba(0,0,0,0.1); transition: all 0.2s ease; }
...
for p in partidos:
    ...
    if st.session_state.partido_abierto == p["partido"]:
        ...
        st.markdown(f"<h4 style='color:#1E90FF; text-align:center;'>Canales de {p['partido']}</h4>", unsafe_allow_html=True)
        ...
        if not p["canales"]:
            ...
            for c in p["canales"]:
                ...
                # Mostrar video solo si coincide con canal abierto
